...
const [form, setForm] = useState({
  ...
  const validateStep = () => {
    ...
    if (step === 3) {
      if (!form.price.trim())   e.price   = "Enter price";
    }
    return e;
  };
  ...
  const handlePhotoAdd = () => {
    const names = ["Living Room", "Bedroom", "Kitchen", "Bathroom", "Balcony", "Exterior"];
    const colors = ["#e8e2d5", "#d4e8d4", "#e8d4d4", "#d4d4e8", "#e8e0d4", "#d8e8e0"];
    if (form.photos.length < 8) {
      const idx = form.photos.length;
      set("photos")([...form.photos, { id: Date.now(), name: names[idx % names.length], color: colors[idx % colors.length] }]);
    }
  };
