  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        ...
        const data = await res.json();
        setProperty(data);
        const simRes = await fetch(`${API.PROPERTY}/api/property/search?type=${data.buyOrSell}&search=${data.category}`);
                ...
                {similarProperties.length === 0 ? (
                  ...
                  similarProperties.map((p) => (
                    <div key={p._id}
                      ...
                      style={{ display: "flex", gap: 0, background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, overflow: "hidden", boxShadow: hoveredSim === p._id ? "0 8px 28px rgba(0,0,0,0.1)" : C.cardShadow, transform: hoveredSim === p._id ? "translateY(-3px)" : "none", transition: "all .25s", cursor: "pointer" }}>
                      ...
                      <div className="sim-body" style={{ padding: "16px 16px 16px 16px", flex: 1 }}>
