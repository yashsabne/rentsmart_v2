...
const PaymentUnlockCard = ({ owner, property, currentUser, C }) => {
  ...
  const handleUnlockPhone = async () => {
    try {
      ...
      const orderRes = await fetch(
        `${API.PAYMENT}/api/payment/create-order`,
        {
          method: "POST",
          headers: {
      ...
      if (!orderRes.ok) {
        toast.error(
          orderData.message ||
          "Payment failed. Please verify your email first."
        );
        return;
      }
        ...
        handler: async function (response) {
          try {
            const verifyRes = await fetch(
              `${API.PAYMENT}/api/payment/verify-payment`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
  ...
  const handleUnlockEmail = () => {
    alert("Free Email Unlock Coming Soon");
  };
    ...
    <div style={{ marginBottom: 24, borderRadius: 28, padding: 24, background: "#fff", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 10px 40px rgba(0,0,0,0.06)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "#6b7280" }}>
            Unlock verified contact details securely and
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: 16, borderRadius: 20, background: "#fafafa", border: "1px solid rgba(0,0,0,0.05)", marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 13, color: "#6b7280" }}>
            {unlocked ? owner?.phone : "Phone Hidden"}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <button onClick={handleUnlockEmail} style={{ width: "100%", padding: 16, borderRadius: 18, border: "1px solid rgba(0,0,0,0.06)", background: "#fff", color: "#111827", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>
          Reveal Owner Email
        </button>

        <button onClick={handleUnlockPhone} disabled={loading || unlocked} style={{ width: "100%", padding: 17, borderRadius: 18, border: "none", background: unlocked ? "#10b981" : "linear-gradient(135deg,#111827,#1f2937)", color: "#fff", fontWeight: 800, cursor: "pointer", fontSize: 15 }}>
